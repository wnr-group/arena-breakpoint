              ...
              return (
                ...
                >
                  {plan.isPopular && (
                    <>
                      <div
                        className={`absolute top-0 left-0 w-full h-1 rounded-t-2xl transition-colors ${isActive ? 'bg-yellow-500' : 'bg-zinc-800'}`}
                      />
                      <div
                        className={`absolute -top-3 right-5 text-[9px] font-black px-3 py-1 rounded-full uppercase tracking-widest shadow-md transition-colors ${isActive ? 'bg-yellow-500 text-black' : 'bg-zinc-800 text-zinc-400'}`}
                      >
                        Most Popular
                      </div>
                    </>
                  ...
                  <div className="flex justify-between items-start mb-4 mt-1">
                    ...
                    <div
                      className={`p-2.5 rounded-xl transition-colors ${isActive ? 'bg-yellow-500/10 text-yellow-500' : 'bg-zinc-900 text-zinc-500'}`}
                    >
                  ...
                  <ul className="space-y-3 mb-6 min-h-30">
                    {plan.features.map((feature, idx) => (
                      <li
                        key={idx}
                        className={`flex items-start text-xs font-medium transition-colors ${isActive ? 'text-gray-200' : 'text-gray-400'}`}
                      >
                        <CheckCircle2
                          className={`mr-2.5 shrink-0 mt-0.5 transition-colors ${isActive ? 'text-yellow-500' : 'text-zinc-600'}`}
                          size={14}
                        />
                        <span>{feature}</span>
                      </li>
