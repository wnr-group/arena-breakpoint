...
export default function OTPVerification({
  ...
  /**
   * Whether the automatic send has already been started for this component.
   *
   * A ref rather than the `otpSent` state it used to check, because state cannot
   * latch across an await. React runs this effect twice on mount in development,
   * and the second run happened while the first `onSendOTP` was still in flight -
   * so `otpSent` was still false, the guard passed, and two requests went out at
   * once. One of them won and the other came back off the resend cooldown, which
   * is why the screen showed "OTP Sent" and "Failed to Send OTP" together.
   *
   * A ref is written synchronously, so the second run sees it immediately.
   */
  const autoSendStartedRef = useRef(false);

  /**
   * A send is on the wire. Same reasoning as above: the button is disabled off
   * `isSending`, but that only updates on the next render, so two fast clicks
   * can both get through. This closes it for every caller rather than trusting
  ...
  useEffect(() => {
    if (!autoSendOnMount || autoSendStartedRef.current) return;
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      // Released in `finally` so a thrown request does not wedge the screen with
      // no way to try again.
