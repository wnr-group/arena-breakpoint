...
export function PromoCodeTable({ promos, onEdit, onDelete, onAdd }: PromoCodeTableProps) {
  if (promos.length === 0) {
    return (
      <div className="p-12 text-center border border-[#e4e4e7] rounded-xl bg-[var(--surface)] space-y-4">
  ...
  return (
    <Card className="bg-[var(--surface)] border-[#e4e4e7] rounded-xl overflow-hidden shadow-2xl w-full overflow-x-auto">
      <div className="p-4 bg-[var(--background)]/40 border-b border-[#e4e4e7] font-black text-xs uppercase text-muted-content tracking-wider">
        Active Promo Code List
      </div>
      <table className="w-full text-left border-collapse text-xs table-fixed min-w-[800px]">
        <thead>
          <tr className="border-b border-[#e4e4e7] bg-[var(--background)]/20 text-secondary-content font-black uppercase text-[11px] tracking-wider select-none">
        ...
        <tbody className="divide-y divide-[#e4e4e7] font-medium">
          {promos.map((row) => (
            <tr key={row.id} className="group hover:bg-[var(--background)]/30 transition-colors">
              ...
              <td className="p-4 text-[#111115] font-bold whitespace-nowrap">
              ...
              <td className="p-4 whitespace-nowrap">
                {row.is_active ? (
                  ...
                  <span className="bg-[var(--background)]/80 text-red-400 border border-[#e4e4e7] text-[9px] font-black px-2 py-1 rounded-lg uppercase">Inactive</span>
                )}
              </td>
              <td className="p-4 w-24 min-w-[96px] whitespace-nowrap">
                <div className="flex justify-end gap-1.5 opacity-0 -translate-x-2 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300">
                  <Button onClick={() => onEdit(row)} variant="ghost" size="icon" className="h-8 w-8 text-muted-content hover:text-primary hover:bg-[#e4e4e7] rounded-md">
                    <Edit2 className="h-3.5 w-3.5" />
                  </Button>
                  <Button onClick={() => onDelete(row.id)} variant="ghost" size="icon" className="h-8 w-8 text-muted-content hover:text-red-400 hover:bg-[#e4e4e7] rounded-md">
