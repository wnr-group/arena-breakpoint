...
export async function createOTPSession(
  ...
  try {
    ...
    if (rateLimitData?.length > 0 && !rateLimitData[0].is_allowed) {
      return {
        success: false,
        // Read on the arena's clock, not the server's. This runs on Vercel,
        // where the host is UTC, so the customer was being told to come back
        // five and a half hours later than the truth.
        message: `Too many OTP requests. Please try again after ${formatClockTime12h(
