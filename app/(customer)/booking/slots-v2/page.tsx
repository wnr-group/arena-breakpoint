  ...
  return (
    <div className="max-w-md md:max-w-7xl mx-auto py-2 px-1 pb-28 md:pb-12">
      ...
      <div className="w-full max-w-md mx-auto flex items-center justify-between pb-6 px-2 select-none">
        ...
        <div className="flex flex-col items-center gap-1">
          <div className="w-5 h-5 rounded-full bg-[#e4e4e7] text-[#52525b] font-black text-[9px] flex items-center justify-center">2</div>
          <span className="text-[8px] font-black uppercase text-primary tracking-wider">Details</span>
        </div>

        <div className="h-0.5 bg-[#e4e4e7] flex-1 mx-2" />
        <div className="flex flex-col items-center gap-1">
          <div className="w-5 h-5 rounded-full bg-[#e4e4e7] text-[#52525b] font-bold text-[9px] flex items-center justify-center border border-[#e4e4e7]">3</div>
          <span className="text-[8px] font-black uppercase text-[#52525b] tracking-wider">Payment</span>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-1 space-y-4 md:space-y-6">
          {/* Device Info */}
          <div className="bg-[#f4f4f5] border border-[#e4e4e7] rounded-xl p-4 flex items-center justify-between gap-4 shadow-md glow-box-hover">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className="p-2.5 bg-[#f4f4f5] border border-[#e4e4e7] text-primary rounded-lg flex-shrink-0">
                <Clock className="h-4 w-4" />
              </div>
              <div className="min-w-0 flex-1">
                <h4 className="font-black text-xs sm:text-sm min-w-0 text-[#111115] uppercase break-words leading-tight">{deviceTypeName || "Gaming Device"}</h4>
                <p className="text-[#3f3f46] text-xs font-bold mt-0.5"><span className="text-primary font-black">₹{hourlyRate || 0}</span> / hour</p>
              </div>
            </div>
          ...
          <Card className="bg-gradient-to-r from-primary/5 via-amber-400/5 to-primary/5 border-primary/20 p-4 glow-box-hover">
            <div className="flex gap-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-[#3f3f46]">Book from 30 minutes to 5 hours. Choose your preferred start time and duration.</p>
              </div>
            </div>
          </Card>
          ...
          <div className="space-y-4 md:hidden">
            <div onClick={() => setMobileCalendarOpen(true)} className="bg-[#f4f4f5] border border-[#e4e4e7] p-4 rounded-xl flex justify-between items-center cursor-pointer glow-box-hover">
              <div className="space-y-1">
                <span className="text-label-enhanced block">Select Date</span>
                <span className="text-sm font-black text-[#111115]">{calendarDay ? calendarDay.toLocaleDateString() : "Choose Date"}</span>
              </div>
              <ChevronRight className="h-4 w-4 text-[#52525b]" />
            </div>

            <div onClick={() => setMobileDurationOpen(true)} className="bg-[#f4f4f5] border border-[#e4e4e7] p-4 rounded-xl flex justify-between items-center cursor-pointer glow-box-hover">
              <div className="space-y-1">
                <span className="text-label-enhanced block">Duration</span>
                <span className="text-sm font-black text-[#111115]">{selectedDurationLabel}</span>
              </div>
              <ChevronRight className="h-4 w-4 text-[#52525b]" />
            </div>

            <div onClick={() => setMobileStartTimeOpen(true)} className="bg-[#f4f4f5] border border-[#e4e4e7] p-4 rounded-xl flex justify-between items-center cursor-pointer glow-box-hover">
              <div className="space-y-1">
            ...
            <Card className="bg-[#f4f4f5] border border-[#e4e4e7] p-5 space-y-4 shadow-xl rounded-xl glow-box-strong">
              <h3 className="text-sm font-black text-[#3f3f46] uppercase tracking-wider border-b border-[#e4e4e7]/60 pb-2">Booking Summary</h3>

              <div className="space-y-2.5 text-sm text-[#3f3f46] border-b border-[#e4e4e7]/60 pb-3">
                <div className="flex justify-between"><span>Date:</span><strong className="text-[#111115] font-bold">{calendarDay ? `${calendarDay.toLocaleDateString()} (${calendarDay.toLocaleDateString('en-US', { weekday: 'short' })})` : "Not Selected"}</strong></div>
                <div className="flex justify-between"><span>Duration:</span><strong className="text-[#111115] font-bold">{selectedDurationLabel}</strong></div>
                ...
                <div className="flex justify-between"><span>Device:</span><strong className="text-xs text-[#111115] uppercase truncate max-w-[200px]">{deviceTypeName || "N/A"}</strong></div>
              </div>

              {/* Player Multiplier Controller */}
              <div className="space-y-2">
                <h4 className="text-[10px] font-black text-[#3f3f46] uppercase tracking-wider">Number of Players</h4>
                <div className="flex items-center justify-between bg-[#f4f4f5] border border-[#e4e4e7] rounded-lg p-2.5">
                  <p className="text-xs text-[#3f3f46] font-bold">{includedPlayers} included • Max {maxPlayers}</p>
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => playerCount > 1 && dispatch(setPlayerCount(playerCount - 1))}
                      disabled={playerCount <= 1}
                      className="w-7 h-7 rounded-md bg-[#e4e4e7] border border-[#e4e4e7] text-[#111115] disabled:opacity-30 flex items-center justify-center hover:border-primary/50 hover:bg-primary/10 transition-all duration-300"
                    >
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="text-base font-black text-[#111115] w-6 text-center">{playerCount}</span>
                    <button
                      type="button"
                      onClick={() => {
              ...
              <div className="space-y-2 text-sm text-[#3f3f46] pt-1">
                <div className="flex justify-between"><span>Base Rate ({selectedDurationLabel})</span><span className="text-[#111115] font-bold">₹{Math.round(baselineSubtotal)}.00</span></div>
                {extraPlayersCount > 0 && (
                ...
                <div className="flex justify-between items-baseline pt-2.5 border-t border-[#e4e4e7] text-[#111115] font-black">
          ...
          <div className="hidden md:grid grid-cols-3 gap-6 items-start">
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-black text-[#52525b] uppercase tracking-widest pl-1">📅 Select Date</h3>
              <Card className="bg-[#f4f4f5] border border-[#e4e4e7] p-4 w-full flex justify-center rounded-2xl glow-box-hover">
            ...
            {/* Duration Selection */}
            <div className="space-y-3">
              <h3 className="text-xs font-black text-[#52525b] uppercase tracking-widest pl-1">⏱️ Duration</h3>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-3 border text-left rounded-xl transition-all duration-300 ${isSelected
                        ...
                        : "bg-[#f4f4f5] border-[#e4e4e7] text-[#3f3f46] hover:border-primary/50 hover:bg-gradient-to-r hover:from-primary/10 hover:to-yellow-400/10 hover:shadow-[0_0_15px_rgba(255,193,7,0.2)]"
                        }`}
                    >
                      <div className="flex justify-between items-center">
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-black text-[#52525b] uppercase tracking-widest pl-1">🕒 Start Time</h3>
              {queryingDb ? (
                <div className="h-96 flex flex-col items-center justify-center gap-2">
                  <Loader2 className="h-5 w-5 text-primary animate-spin" />
                  <p className="text-xs text-[#52525b]">Checking availability...</p>
                </div>
                ...
                <div className="h-96 flex flex-col items-center justify-center gap-2 text-center px-4">
                  ...
                  <p className="text-sm text-[#3f3f46] font-bold">No time slots available</p>
                  <p className="text-xs text-[#52525b]">Try selecting a different date or duration</p>
                </div>
              ) : (
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-3 border text-left rounded-xl transition-all duration-300 text-sm font-bold relative ${!isAvailable
                          ? "bg-[#f4f4f5]/20 border-[#e4e4e7] text-[#a1a1aa] cursor-not-allowed"
                          : isSelected
                            ? "bg-gradient-to-r from-primary via-yellow-400 to-primary border-transparent text-black shadow-[0_4px_20px_rgba(255,193,7,0.4)]"
        ...
        <div className="hidden lg:block w-[360px] flex-shrink-0">
          <Card className="bg-[#f4f4f5] border border-[#e4e4e7] p-6 text-[#111115] space-y-6 shadow-2xl rounded-2xl sticky top-24 glow-box-strong">
            <h3 className="text-sm font-black text-[#3f3f46] uppercase tracking-wider border-b border-[#e4e4e7] pb-3">Booking Summary</h3>

            <div className="space-y-4 text-sm text-[#3f3f46] border-b border-[#e4e4e7] pb-3">
              <div className="flex justify-between">
                <span>Date:</span>
                <strong className="text-[#111115] font-bold">{calendarDay ? `${calendarDay.toLocaleDateString()} (${calendarDay.toLocaleDateString('en-US', { weekday: 'short' })})` : "Not Selected"}</strong>
              </div>
              <div className="flex justify-between">
                <span>Duration:</span>
                <strong className="text-[#111115] font-bold">{selectedDurationLabel}</strong>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                <span>Device:</span>
                <strong className="text-xs text-[#111115] uppercase text-right max-w-[200px] break-words leading-tight max-w-[160px]">{deviceTypeName || "N/A"}</strong>
              </div>
            </div>

            {/* Player Selection Desktop */}
            <div className="border-b border-[#e4e4e7] pb-4">
              <h4 className="text-xs font-black text-[#3f3f46] uppercase tracking-wider mb-3">Number of Players</h4>
              <div className="flex items-center justify-between bg-[#f4f4f5] border border-[#e4e4e7] rounded-lg p-3">
                <div className="flex-1">
                  <p className="text-sm text-[#3f3f46] font-bold">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => playerCount > 1 && dispatch(setPlayerCount(playerCount - 1))}
                    disabled={playerCount <= 1}
                    className="w-7 h-7 rounded-md bg-[#e4e4e7] border border-[#e4e4e7] text-[#111115] disabled:opacity-30 disabled:cursor-not-allowed hover:border-primary/50 hover:bg-primary/10 transition-all duration-300 flex items-center justify-center"
                  >
                    <Minus className="h-3 w-3" />
                  </button>
                  <span className="text-xl font-black text-[#111115] w-8 text-center">{playerCount}</span>
                  <button
                    onClick={() => {
                      if (playerCount >= maxPlayers) {
                        toast.error(`Maximum ${maxPlayers} players only`);
            ...
            <div className="space-y-2.5 text-sm text-[#3f3f46]">
              <div className="flex justify-between">
                <span>Base Rate ({selectedDurationLabel})</span>
                <span className="text-[#111115] font-bold">₹{Math.round(baselineSubtotal)}.00</span>
              </div>
              {extraPlayersCount > 0 && (
                <div className="flex justify-between">
              ...
              <div className="flex justify-between items-baseline pt-3 border-t border-[#e4e4e7] text-[#111115] font-black">
      ...
      {mobileCalendarOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-end md:hidden animate-in fade-in duration-150">
          <div className="bg-[#f4f4f5] border-t border-[#e4e4e7] rounded-t-2xl w-full p-5 space-y-4 animate-in slide-in-from-bottom duration-250">
            <div className="flex justify-between items-center border-b border-[#e4e4e7] pb-2">
              <span className="text-xs font-black uppercase text-[#3f3f46]">Select Date</span>
              <button onClick={() => setMobileCalendarOpen(false)} className="p-1.5 rounded-full bg-[#f4f4f5] text-[#52525b]">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="flex justify-center bg-[#f4f4f5] p-2 rounded-xl">
      ...
      {mobileDurationOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-end md:hidden">
          <div className="bg-[#f4f4f5] border-t border-[#e4e4e7] rounded-t-2xl w-full p-5 space-y-4 max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-[#e4e4e7] pb-2">
              <span className="text-xs font-black uppercase text-[#3f3f46]">Select Duration</span>
              <button onClick={() => setMobileDurationOpen(false)} className="p-1.5 rounded-full bg-[#f4f4f5] text-[#52525b]">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="space-y-1.5">
              {filteredDurations.map((duration) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 border text-left rounded-xl ${isSelected ? "bg-primary text-black border-transparent" : "bg-[#f4f4f5] border-[#e4e4e7] text-[#3f3f46]"
                      }`}
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-bold">{duration.label}</span>
      ...
      {mobileStartTimeOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-end md:hidden">
          <div className="bg-[#f4f4f5] border-t border-[#e4e4e7] rounded-t-2xl w-full p-5 space-y-4 max-h-[75vh] flex flex-col overflow-hidden">
            <div className="flex justify-between items-center border-b border-[#e4e4e7] pb-2 flex-shrink-0">
              <span className="text-xs font-black uppercase text-[#3f3f46]">Select Start Time</span>
              <button onClick={() => setMobileStartTimeOpen(false)} className="p-1.5 rounded-full bg-[#f4f4f5] text-[#52525b]">
                <X className="h-4 w-4" />
              </button>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`p-3 text-center rounded-xl text-xs font-black uppercase transition-all tracking-wider border relative ${!isAvailable
                      ? "bg-[#f4f4f5]/40 border-[#e4e4e7]/40 text-[#a1a1aa] cursor-not-allowed line-through"
                      : isSelected
                        ...
                        : happyHourCheck.hasHappyHour
                          ...
                          : "bg-[#e4e4e7] border-[#e4e4e7] text-[#3f3f46] active:border-[#e4e4e7]"
                      }`}
                  >
